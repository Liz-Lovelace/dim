...
state = {}
state.modifier_keys = {shift = false, ctrl = false, alt = false}

function love.keypressed(key, scancode, isrepeat)
  if key == "lshift" or key == "rshift" then
    state.modifier_keys.shift = true
  elseif key == "lctrl" or key == "rctrl" then
    state.modifier_keys.ctrl = true
  elseif key == "lalt" or key == "ralt" then
    state.modifier_keys.alt = true
  end

  send_input_to_editor(key, state.modifier_keys)
end

function love.keyreleased(key)
  if key == "lshift" or key == "rshift" then
    state.modifier_keys.shift = false
  elseif key == "lctrl" or key == "rctrl" then
    state.modifier_keys.ctrl = false
  elseif key == "lalt" or key == "ralt" then
    state.modifier_keys.alt = false
  end
end
