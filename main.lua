...
function love.load()
  ...
  font = love.graphics.newFont('iosevka-splendid-regular.ttf', 12)
...
function love.draw()
  ...
  love.graphics.print(pane_output.content, 0, 0)
  love.graphics.print(string.format("FPS %d", love.timer.getFPS( )), love.graphics.getWidth() - 40, 0)
  love.graphics.print(string.format("cursor %d %d", pane_output.cursor.x, pane_output.cursor.y), love.graphics.getWidth() - 80, 20)
...
function love_to_tmux_key(love_key, modifier_keys_state)
    ...
    local modifier = ""
    if modifier_keys_state.ctrl then
        modifier = "C-"
    end
    if modifier_keys_state.alt then
        modifier = "M-"
    end
